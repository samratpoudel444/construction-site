  ...
  return (
    <div className="sm:m-16 m-4">
      <div className="flex flex-col gap-16">
        ...
        <div className="sm:grid sm:grid-cols-4 sm:gap-8 flex flex-col gap-8">
          {projectName.map((data, index) => (
            <div
              key={index}
              className="relative hover:scale-110 hover:transition sm:h-64 sm:w-64 w-[95%] h-[40%] gap-4"
            >
                <button></button>
              <img src={data.url} alt={data.name} className="h-full w-full" />
              <p className="absolute w-full bg-gray-600 text-white p-2">{data.name}</p>
            </div>
          ))}
        </div>
      </div>
